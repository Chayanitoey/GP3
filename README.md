...
The prompt assigned for GPT-3 is **'What is the product that the following reviews talk about?'**, from the review input, GPT3 will generate topic or product that is most relevant and best respresentation of that cluster. 
...
The topics that were generated by GPT-3 are clearly giving us the advantage and save time in manual labeling, however, there are some duplicates that would need human inteligence to support and further explore (for example 'tea' in cluster #28,#37 and #38). 
